...
*/
(function(f){f.OT||(f.OT={});OT.properties={version:"v2.0.7",build:"c72c0bb",debug:"false",websiteURL:"http://www.tokbox.com",cdnURL:"/js/opentok",loggingURL:"http://hlg.tokbox.com/prod",apiURL:"http://anvil.opentok.com",supportSSL:"true",cdnURLSSL:"https://swww.tokbox.com",loggingURLSSL:"https://hlg.tokbox.com/prod",apiURLSSL:"https://anvil.opentok.com"}})(window);
(function(){var f=this,d=f._,b={},a=Array.prototype,c=Object.prototype,e=a.push,h=a.slice,j=a.concat,i=c.toString,k=c.hasOwnProperty,n=a.forEach,m=a.map,o=a.reduce,q=a.reduceRight,p=a.filter,s=a.every,u=a.some,t=a.indexOf,y=a.lastIndexOf,c=Array.isArray,z=Object.keys,x=Function.prototype.bind,g=function(a){if(a instanceof g)return a;if(!(this instanceof g))return new g(a);this._wrapped=a};"undefined"!==typeof exports?("undefined"!==typeof module&&module.exports&&(exports=module.exports=g),exports._=
...
arguments)}};g.keys=z||function(a){if(a!==Object(a))throw new TypeError("Invalid object");var b=[],c;for(c in a)g.has(a,c)&&(b[b.length]=c);return b};g.values=function(a){var b=[],c;for(c in a)g.has(a,c)&&b.push(a[c]);return b};g.pairs=function(a){var b=[],c;for(c in a)g.has(a,c)&&b.push([c,a[c]]);return b};g.invert=function(a){var b={},c;for(c in a)g.has(a,c)&&(b[a[c]]=c);return b};g.functions=g.methods=function(a){var b=[],c;for(c in a)g.isFunction(a[c])&&b.push(c);return b.sort()};g.extend=function(a){r(h.call(arguments,
...
(function(f){function d(b,a,c){this.code=b;this.reason=a;this.signal=c}OT.Messenger=function(b,a){function c(){OT.debug("WebSocket connected");clearTimeout(t);t=setTimeout(function(){B("Failure","reason","Connection to the server timed out waiting to receive connected message.");w.trigger("exception","Connection to the server timed out waiting to receive connected message.",OT.ExceptionCodes.CONNECTION_TIMEOUT)},15E3)}function e(a){OT.debug("WebSocket error: "+a.data);w.trigger("exception","TB.Socket Error :: The socket to "+
b+" received an error: "+a.data,OT.ExceptionCodes.CONNECT_FAILED)}function h(){clearTimeout(y);w.trigger("ConnectionClosed",{reason:z?"clientDisconnected":"networkDisconnected"});A=null;r=!1}function j(a){OT.warn("WebSocket message recieved: "+a.data);i(a.data)}function i(b){var c=JSON.parse(b);switch(c.type){case OT.WebSocketMessageType.CONNECTED_TO_WEBSOCKET_SERVER:n(c.payload);break;case OT.WebSocketMessageType.WEBSOCKET_PONG:C=+Date.now();break;case OT.WebSocketMessageType.CONNECT_TO_SESSION:clearTimeout(t),
r=!0,A=c.payload.connectionId,k();default:if(m.hasOwnProperty(c.type)){var d=m[c.type].type;OT.debug("Received "+d);!r&&m[c.type].afterConnected?g.push(b):w.trigger(d,a.wrangle(c.payload))}else OT.debug("Message type "+c.type+" was not handled.");if(c.type===OT.WebSocketMessageType.CONNECT_TO_SESSION)for(;0<g.length;)i(g.shift())}}function k(){var a;a=C?+Date.now()-C>=q:!1;a?(OT.error("We seem to have lost connectivity!"),h()):(w.sendMessage(OT.WebSocketMessage.pingMessage(w.connectionId,"ping!")),
y=setTimeout(k,o))}function n(a){w.connectionId=a.connectionId;B("Success","webSocketServerUrl",_webSocketUrl,{connectionId:a.connectionId});clearTimeout(t);t=setTimeout(function(){B("Failure","reason","Connection to the server timed out waiting to receive connected message.");w.trigger("exception","Connection to the server timed out fetching the session state.",OT.ExceptionCodes.CONNECTION_TIMEOUT)},15E3);w.sendMessage(OT.WebSocketMessage.connectToSessionMessage(p,OT.APIKEY,s,u.requireConnectionObjects,
...
{type:"JSEPMessageReceived",afterConnected:!0};m[OT.WebSocketMessageType.JSEP_SUBSCRIBE]={type:"JSEPMessageReceived",afterConnected:!0};m[OT.WebSocketMessageType.JSEP_UNSUBSCRIBE]={type:"JSEPMessageReceived",afterConnected:!0};m[OT.WebSocketMessageType.SIGNAL]={type:"SignalReceived",afterConnected:!0};var o=25E3,q=3*o-100;OT.$.eventing(this);this.connectionId=this.webSocket=this.onConnectionClosed=this.onSessionConnected=null;var p,s,u,t,y,z=!1,x=new OT.Analytics,g=[],r=!1,A,C=null,B=function(a,b,
c,d){a={action:"webSocketConnection",variation:a,payload_type:b,payload:c,session_id:p,partner_id:u.partnerId,widget_id:u.widgetId,widget_type:"Controller"};d&&(a=OT._.extend(d,a));x.logEvent(a)};this.connect=function(a,d,g){p=a;s=d;u=g;_webSocketUrl="wss://"+b+":443/rumorwebsockets";B("Attempt","webSocketServerUrl|userAgent|sdkVersion|chromeFrame",[_webSocketUrl,navigator.userAgent,OT.properties.version,f.externalHost?"yes":"no"].map(function(a){return a.replace("|","\\|")}).join("|"));this.webSocket=
new WebSocket(_webSocketUrl);this.webSocket.onopen=c;this.webSocket.onclose=h;this.webSocket.onerror=e;this.webSocket.onmessage=j;t=setTimeout(function(){B("Failure","reason","Connection to the server timed out waiting to receive connected message.");w.trigger("exception","Connection to the server timed out.",OT.ExceptionCodes.CONNECTION_TIMEOUT)},15E3)};this.disconnect=function(){z=!0;this.webSocket&&(this.webSocket.close(),this.webSocket=null)};this.sendMessage=function(a){OT.warn("Sending WebSocket message: "+
JSON.stringify(a));this.webSocket.send(JSON.stringify(a))};this.signal=function(a){var b=new OT.WebSocketMessage.signalMessage(p,A,a),a=function(){if(b.onError)b.onError(new d(b.error.code,b.error.reason,b.signal))};if(b.valid){OT.warn("Sending WebSocket signal: "+b.toJson());try{if(this.webSocket.send(b.toJson()),b.onSuccess)b.onSuccess(b.signal)}catch(c){a()}}else a()};var w=this}})(window);
...
m,o,q="new",p=[];OT.$.eventing(this);navigator.mozGetUserMedia&&(a.iceServers=[]);var s=OT._.bind(function(a,b){if(p.length)p[0](a,b)},this),u=OT._.bind(function(){if(!f){try{OT.debug('Creating peer connection config "'+JSON.stringify(a)+'".');f=new d(a,{optional:[{DtlsSrtpKeyAgreement:true}]})}catch(b){OT.error("Failed to create PeerConnection, exception: "+b.message);return null}n.peerConnection=f;f.onicecandidate=c(s);f.onaddstream=OT._.bind(y,this);f.onremovestream=OT._.bind(z,this);if(f.onsignalingstatechange!==
void 0)f.onsignalingstatechange=OT._.bind(t,this);else if(f.onstatechange!==void 0)f.onstatechange=OT._.bind(t,this)}return f},this),t=function(a){if((a=a.target.signalingState||a.target.readyState)&&a.toLowerCase()!==q){q=a.toLowerCase();OT.debug("PeerConnection.stateChange: "+q);switch(q){case "closed":if(n)n.peerConnection=null;f=null;this.trigger("close")}}},y=function(a){this.trigger("streamAdded",a.stream,a.type==="audio"||a.type==="video"?a.type:"merged")},z=function(a){this.trigger("streamRemoved",
a.stream,a.type==="audio"||a.type==="video"?a.type:"merged")},x=function(a){a=new b(a.sdp);u();_remoteDescriptionType=a.type;_remoteDescription=a;h(f,a,function(a){s(OT.WebSocketMessageType.JSEP_ANSWER,{sdp:a})},function(a,b){r("PeerConnection.offerProcessor "+a+": "+b)})},g=function(){OT.debug("PeerConnection.processSubscribe: Sending offer to subscriber.");u();j(f,function(a){m=a;s(OT.WebSocketMessageType.JSEP_OFFER,{sdp:m})},function(a,b){r("PeerConnection.suscribeProcessor "+a+": "+b)})},r=OT._.bind(function(a){OT.error(a);
this.trigger("error",a)},this);this.addLocalStream=function(a){u();f.addStream(a)};this.disconnect=function(){n=null;if(f){var a=f.signalingState||f.readyState;a&&a.toLowerCase()!=="closed"&&f.close();f=null}this.off()};this.processMessage=function(a,c){OT.debug("PeerConnection.processMessage: Received "+a+" from "+c.fromAddress);OT.debug(c);switch(a){case OT.WebSocketMessageType.JSEP_SUBSCRIBE:g.call(this,c);break;case OT.WebSocketMessageType.JSEP_OFFER:x.call(this,c);break;case OT.WebSocketMessageType.JSEP_ANSWER:if(c.sdp){o=
new b(c.sdp);_remoteDescriptionType=o.type;_remoteDescription=o;f.setRemoteDescription(o);n.processPending()}else OT.error("PeerConnection.processMessage: Weird message, no SDP.");break;case OT.WebSocketMessageType.JSEP_CANDIDATE:n.process(c);break;default:OT.debug("PeerConnection.processMessage: Received an unexpected message of type "+a+" from "+c.fromAddress+": "+JSON.stringify(c))}return this};this.registerMessageDelegate=function(a){p.push(a)};this.unregisterMessageDelegate=function(a){OT._.without(p,
a)};Object.defineProperty(this,"remoteStreams",{get:function(){var a;if(f){if(f.getRemoteStreams)a=f.getRemoteStreams();else if(f.remoteStreams)a=f.remoteStreams;else throw Error("Invalid Peer Connection object implements no method for retrieving remote streams");a=Array.prototype.slice.call(a)}else a=[];return a}})}})(window);
(function(){var f={};OT.PeerConnections={add:function(d,b,a){d=d.id+"_"+b.id;(b=f[d])||(b=f[d]={count:0,pc:new OT.PeerConnection(a)});b.count+=1;return b.pc},remove:function(d,b){var a=d.id+"_"+b.id,c=f[a];c&&(c.count-=1,0===c.count&&(c.pc.disconnect(),delete f[a]))}}})(window);
...
(function(){OT.Publisher=function(){if(OT.checkSystemRequirements()){var f=OT.Publisher.nextId(),d,b,a,c,e,h,j={},i=!1,k,n,m,o,q=new OT.Analytics,p;OT.$.eventing(this);OT.StylableComponent(this,{showMicButton:!0,showSettingsButton:!0,showCameraToggleButton:!0,nameDisplayMode:"auto",buttonDisplayMode:"auto",backgroundImageURI:null});var s=function(a,b,d,e){q.logEvent({action:a,variation:b,payload_type:d,payload:e,session_id:h?h.sessionId:null,connection_id:h&&h.connected?h.connection.connectionId:
null,partner_id:h?h.apiKey:OT.APIKEY,streamId:c?c.id:null,widget_id:f,widget_type:"Publisher"})},u=function(){OT.debug("OT.Publisher.onLoaded");s("publish","Success","streamType","WebRTC");b.loading=!1;E.call(this);i=!0;this.trigger("loaded",this)},t=function(a){s("publish","Failure","reason","Publisher PeerConnection Error: "+a);OT.handleJsException("Publisher PeerConnection Error: "+a,OT.ExceptionCodes.P2P_CONNECTION_FAILED,{session:h,target:this})},y=function(c){OT.debug("OT.Publisher.onStreamAvailable");
this.dispatchEvent(new OT.Event(OT.Event.names.ACCESS_ALLOWED,!1));e&&(e.stop(),e=null);e=c;m=new OT.Microphone(e,!k.publishAudio);this.publishVideo(k.publishVideo);a=new OT.VideoElement({attributes:{muted:!0}});a.on({streamBound:OT._.bind(u,this),timeout:OT._.bind(t,this),error:OT._.bind(r,this)}).bindToStream(e);b.video=a;OT.DOMComponentCleanup.add(this,a)},z=function(a){var c=function(a,b){OT.error("OT.Publisher.onStreamAvailableError "+a);s("publish","Failure","reason","Publisher failed to access camera/mic: "+
a);OT.handleJsException("Publisher failed to access camera/mic: "+a,2E3,{session:h,target:b})};switch(a.code){case 1:OT.error("OT.Publisher.onStreamAvailableError Permission Denied");s("publish","Failure","reason","Publisher Access Denied: Permission Denied ("+a.code+")");var d=new OT.Event(OT.Event.names.ACCESS_DENIED);this.dispatchEvent(d,function(){!d.isDefaultPrevented()&&b&&b.destroy()});break;case "NO_CONSTRAINTS":b&&b.destroy();c("No constaints were provided",this);break;default:b&&b.destroy(),
c("Unknown Reason ("+a.code+")",this)}},x=function(){s("accessDialog","Opened","","");this.dispatchEvent(new OT.Event(OT.Event.names.ACCESS_DIALOG_OPENED,!1))},g=function(){s("accessDialog","Closed","","");this.dispatchEvent(new OT.Event(OT.Event.names.ACCESS_DIALOG_CLOSED,!1))},r=function(a,b){OT.error("OT.Publisher.onVideoError");var c=b+(a?" ("+a+")":"");s("stream",null,"reason","Publisher while playing stream: "+c);OT.handleJsException("Publisher error playing stream: "+c,2E3,{session:h,target:this})},
A=function(a){OT.debug("OT.Subscriber has been disconnected from the Publisher's PeerConnection");this.cleanupSubscriber(a.remoteConnection.id)},C=function(a,b,c){s("publish","Failure","reason|hasRelayCandidates",["Publisher PeerConnection with connection "+c.remoteConnection.id+" failed: "+b,c.hasRelayCandidates].join("|"));OT.handleJsException("Publisher PeerConnection Error: "+b,2E3,{session:h,target:this});delete j[c.remoteConnection.id]},B=function(a){var b=j[a.id];if(!b){var d=OT.$.now();s("createPeerConnection",
"Attempt","","");b=j[a.id]=new OT.PublisherPeerConnection(a,h,c,e);b.on({connected:function(){s("createPeerConnection","Success","pcc|hasRelayCandidates",[parseInt(OT.$.now()-d,10),b.hasRelayCandidates].join("|"))},disconnected:OT._.bind(A,this),error:OT._.bind(C,this)})}return b},w=function(a){if(!1===a)return"off";a=this.getStyle("buttonDisplayMode");return!1===a?"on":a},E=function(){o=(new OT.Chrome({parent:b.domElement})).set({name:new OT.Chrome.NamePanel({name:k.name,mode:this.getStyle("nameDisplayMode")}),
...
(o.destroy(),o=null);this.disconnect();m=null;a&&(a.destroy(),a=null);e&&(e.stop(),e=null);b&&(b.destroy(),b=null);this.session&&this._.unpublishFromSession(this.session);p&&clearInterval(p);c=d=null;i=!1;_properties=h=null;this.trigger("destroyed",this);this.off();return this};this.disconnect=function(){for(var a in j)this.cleanupSubscriber(a)};this.cleanupSubscriber=function(a){var b=j[a];b&&(b.destroy(),delete j[a],s("disconnect","PeerConnection","subscriberConnection",a))};this.processMessage=
...
(function(){OT.Subscriber=function(f,d){var b=uuid(),a=f||b,c,e={},h,j,i,k=d.session,n=!1,m,o,q,p=OT._.clone(d),s=new OT.Analytics,u=50;if(k){OT.$.eventing(this);OT.StylableComponent(this,{nameDisplayMode:"auto",buttonDisplayMode:"auto",backgroundImageURI:null});var t=function(a,c,d,e){s.logEvent({action:a,variation:c,payload_type:d,payload:e,stream_id:j?j.id:null,session_id:k?k.sessionId:null,connection_id:k&&k.connected?k.connection.connectionId:null,partner_id:k&&k.connected?k.sessionInfo.partnerId:
null,widget_id:b,widget_type:"Subscriber"})},y=function(){n||0===e.length||(OT.debug("OT.Subscriber.onLoaded"),n=!0,m=OT.$.now(),t("createPeerConnection","Success","pcc|hasRelayCandidates",[parseInt(m-o,10),i&&i.hasRelayCandidates].join("|")),q=setInterval(this.recordQOS,3E4),c.loading=!1,A.call(this),this.trigger("loaded",this),t("subscribe","Success","streamId",j.id))},z=function(){OT.debug("OT.Subscriber has been disconnected from the Publisher's PeerConnection");this.disconnect()},x=function(a,
b){n||t("createPeerConnection","Failure","reason|hasRelayCandidates",["Subscriber PeerConnection Error: "+b,i&&i.hasRelayCandidates].join("|"));this.disconnect();t("subscribe","Failure","reason","Subscriber PeerConnection Error: "+b);OT.handleJsException("Subscriber PeerConnection Error: "+b,OT.ExceptionCodes.P2P_CONNECTION_FAILED,{session:k,target:this});c&&c.addError(b)},g=function(a,b){OT.debug("OT.Subscriber.onRemoteStreamAdded");this.subscribeToAudio(p.subscribeToAudio);this.subscribeToVideo(p.subscribeToVideo);
var d=new OT.VideoElement;d.setAudioVolume(u);d.on({streamBound:OT._.bind(y,this),timeout:OT._.bind(x,this),error:OT._.bind(x,this)});"audio"===b&&OT.$.addClass(d.domElement,"OT_hidden-audio");d.bindToStream(a);c.video=d;OT.DOMComponentCleanup.add(this,d);e[b]=d;"audio"!==b&&this.streamOrientationDidChange(j.orientation.width,j.orientation.height,j.orientation.videoOrientation);t("createPeerConnection","StreamAdded","","");this.trigger("streamAdded",this)},r=function(a){OT.debug("OT.Subscriber.onStreamRemoved");
for(var b in e)e[b].stream==a&&(e[b].destroy(),delete e[b]);this.trigger("streamRemoved",this)},A=function(){h=(new OT.Chrome({parent:c.domElement})).set({name:new OT.Chrome.NamePanel({name:p.name,mode:this.getStyle("nameDisplayMode")}),opentokButton:new OT.Chrome.OpenTokButton({infoHref:OT.$.moreInfoLink(k)})}).on({muted:function(){},unmuted:function(){}})};this.recordQOS=function(){n&&(k&&k.connected)&&s.logQOS({widget_type:"Subscriber",stream_type:"WebRTC",session_id:k.sessionId,connectionId:k.connection.connectionId,
...
p.style&&this.setStyle(p.style,null,!0);p.audioVolume&&this.setAudioVolume(p.audioVolume);p.subscribeToAudio=OT.$.castToBoolean(p.subscribeToAudio,!0);p.subscribeToVideo=OT.$.castToBoolean(p.subscribeToVideo,!0);c=new OT.WidgetView(a,p);o=OT.$.now();j.connection.id!==k.connection.id?(t("createPeerConnection","Attempt","",""),i=new OT.SubscriberPeerConnection(j.connection,k,j),i.on({disconnected:OT._.bind(z,this),error:OT._.bind(x,this),remoteStreamAdded:OT._.bind(g,this),remoteStreamRemoved:OT._.bind(r,
this)})):g.call(this,k.getPublisherForStream(j)._.webRtcStream,"merged");t("subscribe","Attempt","streamId",j.id);return this};this.destroy=function(){OT.DOMComponentCleanup.remove(this);this.disconnect();h&&(h.destroy(),h=null);c&&(c.destroy(),c=null);j&&t("unsubscribe",null,"streamId",j.id);j=a=null;clearInterval(q);p=k=null;this.trigger("destroyed",this);this.off();return this};this.disconnect=function(){n=!1;for(var a in e)e[a].destroy();e={};i&&(i.destroy(),i=null,t("disconnect","PeerConnection",
...
(function(){OT.Session=function(f){function d(a){for(var b in i){var c=i[b];if(a===c.streamId)return c}return null}function b(a){return!a.connection||!j[a.connection.connectionId]?(OT.warn("Received a stream for a connection that doesn't exist"),OT.debug(a),!1):!0}function a(a){n[a.id]=a}if(OT.checkSystemRequirements()){var c=!1,e=!1,h,j={},i={},k={},n={},m,o,q,p={},s=uuid(),u={},t=new OT.Analytics;OT.$.eventing(this);var y=function(d){c=!0;e=!1;h=d.connectionId;p=new OT.Capabilities(d.permissions);
d.connections.forEach(function(a){j[a.connectionId]=a});d.streams=OT._.filter(d.streams,function(a){return b(a)});d.streams.forEach(a);this.dispatchEvent(new OT.SessionConnectEvent(OT.Event.names.SESSION_CONNECTED,d.connections,d.streams,d.archives))},z=function(a){e=c=!1;OT.error(a.reason);this.trigger("sessionConnectFailed",a.reason);switch(a.code){case 409:TB.handleJsException("TB.SessionConnectionFailed :: The P2P session already has two participants.",OT.ExceptionCodes.CONNECT_REJECTED,{session:this});
...
i={};u={};for(var c in k)k[c].destroy();k={}}},this))},r=function(a){j[a.connection.connectionId]=a.connection;a.connections=[a.connection];delete a.connection;this.dispatchEvent(new OT.ConnectionEvent(OT.Event.names.CONNECTION_CREATED,a.connections))},A=function(a){h!==a.connection.connectionId&&delete j[a.connection.connectionId];for(var b in i)i[b].cleanupSubscriber(a.connection.connectionId);a.connections=[a.connection];delete a.connection;this.dispatchEvent(new OT.ConnectionEvent(OT.Event.names.CONNECTION_DESTROYED,
...
K=function(a){u[a.guid]=OT._.bind(function(){this.unpublish(a)},this);a.on("destroyed",u[a.guid])},v=function(){TB.log("connectToMessenger");this.properties={requireConnectionObjects:!0};var a=function(a){return function(b){return function(){return b.apply(a,arguments)}}}(this);q=(new OT.Messenger(this.sessionInfo.messagingServer,new OT.SessionMessageWrangler(this))).on({SessionConnected:a(y),SessionConnectFailed:a(z),ConnectionClosed:a(g),ConnectionCreated:a(r),ConnectionDestroyed:a(A),StreamRegistered:a(C),
StreamCreated:a(B),StreamModified:a(w),StreamDestroyed:a(E),JSEPMessageReceived:a(F),SignalReceived:a(J),exception:a(x)});q.connect(f,o,{requireConnectionObjects:this.properties.requireConnectionObjects,p2pEnabled:this.sessionInfo.p2pEnabled,widgetId:s,partnerId:m})},L=function(){e&&OT.SessionInfo.get(this,OT._.bind(M,this),function(){e=!1})},M=function(a){e&&(this.sessionInfo=a,this.sessionInfo.partnerId&&this.sessionInfo.partnerId!=m?(m=this.sessionInfo.partnerId,TB.handleJsException("Authentication Error: The apiKey passed into the session.connect method does not match the apiKey in the token or session you are trying to connect to.",
OT.ExceptionCodes.AUTHENTICATION_ERROR,{session:this})):v.call(this))};this.logEvent=function(a,b,c,d){t.logEvent({action:a,variation:b,payload_type:c,payload:d,session_id:f,partner_id:m,widget_id:s,widget_type:"Controller"})};this.connect=function(a,b){c?OT.warn("OT.Session: Cannot connect, already connected."):e?OT.warn("OT.Session: Cannot connect, already connecting."):(e=!0,H.call(this),m=a.toString(),0===OT.APIKEY.length&&(OT.APIKEY=m),o=b,L.call(this))};this.disconnect=function(){e=!1;q&&(q.disconnect(),
q=null)};this.publish=function(a,b){var d;if(!c)throw t.logError(1010,"tb.exception","We need to be connected before you can publish",null,{action:"publish",variation:"Failure",payload_type:"reason",payload:"We need to be connected before you can publish",session_id:f,partner_id:m,widgetId:s,widget_type:"Controller"}),d="We need to be connected before you can publish",OT.error(d),Error(d);if(1!==p.publish)return t.logEvent({action:"publish",variation:"Failure",payload_type:"reason",payload:"This token does not allow publishing. The role must be at least `publisher` to enable this functionality",
session_id:f,partner_id:m,widgetId:s,widget_type:"Controller"}),TB.handleJsException("This token does not allow publishing. The role must be at least `publisher` to enable this functionality",OT.ExceptionCodes.UNABLE_TO_PUBLISH,{session:this}),null;if(!a||"string"===typeof a)a=OT.initPublisher(this.apiKey,a,b);else if(a instanceof OT.Publisher||a instanceof OT.flash.Publisher)"session"in a&&(a.session&&"sessionId"in a.session)&&(a.session.sessionId===this.sessionId?OT.warn("Cannot publish "+a.guid+
